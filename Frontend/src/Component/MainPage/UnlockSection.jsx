...
useEffect(() => {
    async function loadLogos() {
      const response = await fetch(`${import.meta.env.VITE_DATA_API_URL}/api/brands`);
      const brands = await response.json();

      const logos = Object.fromEntries(
        brands.map(b => [b.name, b.logo_url])
      );
  ...
  const [countryNetworkData, setCountryNetworkData] = useState([]);
useEffect(() => {
    async function loadCountries() {
      const response = await fetch(`${import.meta.env.VITE_DATA_API_URL}/api/countries`);
      const countries = await response.json();

      // Directly use backend data (same structure)
      setCountryNetworkData(countries);
    }

    loadCountries();
  }, []);
  const [tacRouterDB, setTacRouterDB] = useState({});

  useEffect(() => {
    async function loadTacDB() {
      const res = await fetch(`${import.meta.env.VITE_DATA_API_URL}/api/tac-router`);
      const data = await res.json();

      // Convert Array → Object (just like your hardcoded version)
      ...
      <section className="unlock-section">
        <div className="unlock-container">
          <div className="unlock-content">
            <h1>
              <span className="highlight">Unlock Your Router, WiFi & MiFi</span>{" "}
              Today With{" "}
            ...
            <p>
              Looking to{" "}
              <strong>unlock your router, modem, or MiFi device</strong>{" "}
              quickly, safely, and for free? This all-in-one{" "}
              <strong>router unlock guide</strong> explains every working method
              from <strong>IMEI unlock code generators</strong> and{" "}
              <strong>DC-Unlocker alternatives</strong> to professional{" "}
              <strong>4G and 5G router unlocking tools</strong>. Whether you own
              a <strong>Huawei</strong>, <strong>ZTE</strong>,{" "}
              <strong>D-Link</strong>, <strong>Nokia</strong>,{" "}
              <strong>Telstra</strong>, <strong>FIBOCOM</strong>,{" "}
              <strong>QUECTEL</strong>, <strong>GreenPacket</strong>,{" "}
              <strong>MEIGLINK</strong>, <strong>Soyealink</strong>,{" "}
              <strong>AVXAV</strong>, <strong>AURORA</strong>,{" "}
              <strong>TD TECH</strong>, <strong>BROVI</strong>, or{" "}
              <strong>FLYBOX</strong> Wi-Fi router, you can easily{" "}
              <strong>remove the SIM lock</strong> and use it with any mobile
              carrier worldwide. Follow our step-by-step process to get your{" "}
              <strong>unlocked 4G/5G router or MiFi</strong> working within
                        ...
                        <div>
                          ...
                          <section>
                            ...
                            <p>
                              ...
                              a 8 or 16-digit unlock code to remove network
                          ...
                          <section>
                            ...
                            <p>
                              2.2 If an order is cancelled or not fulfilled, the
                              amount will be refunded after deducting PayPal
                              transaction fees. Please note that PayPal charges
                              a transaction fee of 5.9% of the total amount,
                              which will be subtracted from the refund.
                            </p>
                          </section>
                          ...
                          <section>
                            ...
                            <p>
                              6.1 All services are covered by our Money Back
